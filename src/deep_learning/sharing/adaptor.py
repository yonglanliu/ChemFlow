...
class ResidualAdaptor(nn.Module):
    ...
    def __init__(
        ...
        if bottleneck <= 0:
            raise ValueError(
                f"bottleneck must be positive, but got {bottleneck}."
            )

        if not 0.0 <= dropout < 1.0:
            raise ValueError(
                f"dropout must be in [0, 1), but got {dropout}."
            )

        if gate_fn not in {"tanh", "sigmoid"}:
            raise ValueError(
                f"Unsupported gate function '{gate_fn}'. "
                "Expected 'tanh' or 'sigmoid'."
            )
    ...
    def _compute_gate(self) -> torch.Tensor:
        ...
        if not self.gate:
            raise RuntimeError(
                "_compute_gate() was called while gating is disabled."
            )
        ...
        raise RuntimeError(
            f"Unsupported gate function: {self.gate_fn}"
        )
    ...
    def forward(
        ...
        if z.shape[-1] != self.dim:
            raise ValueError(
                "The last dimension of the adaptor input must be "
                f"{self.dim}, but got {z.shape[-1]}."
            )
