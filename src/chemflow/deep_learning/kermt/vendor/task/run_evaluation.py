...
from chemflow.deep_learning.kermt.vendor.kermt.util.utils import get_class_sizes, get_data, split_data, get_task_names, get_loss_func
from chemflow.deep_learning.kermt.vendor.kermt.util.utils import load_checkpoint
from chemflow.deep_learning.kermt.vendor.task.predict import evaluate_predictions
from chemflow.deep_learning.kermt.vendor.kermt.util.metrics import get_metric_func
from chemflow.deep_learning.kermt.vendor.kermt.util.nn_utils import param_count_trainable, param_count_total
from chemflow.deep_learning.kermt.vendor.task.predict import predict
...
def run_evaluation(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    if args.cuda:
        torch.cuda.set_device(args.gpu if args.gpu is not None else 0)
