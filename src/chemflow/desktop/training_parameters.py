...
GRAPHORMER = ModelParameters(
    ...
    property_common() + (
        ...
        choice("TrainingConfig.regression_loss", "mse", ("l2", "mse", "mae", "huber", "nll", "gaussian_nll"), "Regression optimization loss."),
        number("TrainingConfig.huber_delta", 1.0, "Huber quadratic-to-linear transition.", 1e-8, 1e6),
        number("TrainingConfig.nll_scale", 1.0, "Fixed Laplace scale for NLL loss.", 1e-8, 1e6),
        number("TrainingConfig.gaussian_nll_variance", 1.0, "Fixed variance for Gaussian NLL loss.", 1e-8, 1e6),
    ),
)
...
CHEMELEON = ModelParameters(
    ...
    property_common() + (
        ...
        choice("CheMeleonTrainingConfig.regression_loss", "mse", ("l2", "mse", "mae", "huber", "nll", "gaussian_nll"), "Loss for standardized regression targets."),
        number("CheMeleonTrainingConfig.huber_delta", 1.0, "Huber transition point in standardized-target units.", 1e-8, 1e6),
        number("CheMeleonTrainingConfig.nll_scale", 1.0, "Fixed Laplace scale for NLL loss.", 1e-8, 1e6),
        number("CheMeleonTrainingConfig.gaussian_nll_variance", 1.0, "Fixed variance for Gaussian NLL loss.", 1e-8, 1e6),
...
CHEMPROP = ModelParameters(
    "chemprop", "Chemprop v2 directed message-passing neural-network baseline.", ".toml",
    (
        text("BaseConfig.workdir", "./chemprop_run", "Directory receiving checkpoints, predictions, TensorBoard events, logs, and the launch manifest."),
        choice("BaseConfig.task", "regression", ("regression",), "The current ChemFlow Chemprop adapter supports regression."),
        text("DatasetConfig.dataset_path", "./dataset.csv", "Training CSV available on the machine running the job."),
        text("DatasetConfig.test_dataset_path", "", "Optional independent test CSV; set test fraction to zero when supplied."),
        text("DatasetConfig.smiles_column", "SMILES", "Column containing molecular SMILES strings."),
        items("DatasetConfig.target_column", ["target"], "One or more regression targets; missing labels are masked during multitask training."),
        choice("DatasetConfig.split_type", "scaffold_balanced", SPLITS, "Generated or predefined train/validation/test split."),
        text("DatasetConfig.split_column", "", "Column containing train/val/test labels for a predefined split."),
        number("DatasetConfig.val_fraction", 0.1, "Fraction assigned to validation for generated splits.", 0.0, 0.9),
        number("DatasetConfig.test_fraction", 0.1, "Internal test fraction; use zero with an independent test CSV.", 0.0, 0.9),
        integer("ChempropConfig.message_hidden_dim", 300, "Hidden size of directed bond messages.", 1, 1_000_000),
        integer("ChempropConfig.depth", 3, "Number of message-passing steps.", 1, 100),
        number("ChempropConfig.dropout", 0.0, "Dropout in message-passing and feed-forward layers.", 0.0, 0.95),
        choice("ChempropConfig.activation", "RELU", ("RELU", "GELU", "SILU", "TANH"), "Activation function."),
        choice("ChempropConfig.aggregation", "norm", ("norm", "mean", "sum"), "Atom-to-molecule aggregation method."),
        number("ChempropConfig.aggregation_norm", 100.0, "Divisor used by normalized-sum aggregation.", 1e-8, 1e12),
        integer("ChempropConfig.ffn_hidden_dim", 300, "Prediction-head hidden width.", 1, 1_000_000),
        integer("ChempropConfig.ffn_num_layers", 1, "Number of hidden prediction-head layers under Chemprop v2 semantics.", 1, 100),
        boolean("ChempropConfig.batch_norm", False, "Apply batch normalization after molecular aggregation."),
        items("ChempropConfig.molecule_featurizers", [], "Optional extra features: morgan_binary, morgan_count, rdkit_2d, v1_rdkit_2d, or v1_rdkit_2d_normalized."),
        boolean("ChempropConfig.no_descriptor_scaling", False, "Disable scaling of optional molecule-level descriptors."),
        integer("TrainingConfig.seed", 42, "Dataset-splitting seed."),
        integer("TrainingConfig.pytorch_seed", 42, "Model initialization and PyTorch seed."),
        boolean("TrainingConfig.resume", False, "Continue from model_*/checkpoints/last.ckpt using Chemprop's weights-only checkpoint loading."),
        text("TrainingConfig.resume_checkpoint", "", "Optional checkpoint path; leave empty to discover last.ckpt below the work directory."),
        choice("TrainingConfig.accelerator", "auto", ("auto", "cpu", "gpu", "mps"), "Lightning compute accelerator."),
        text("TrainingConfig.devices", "auto", "Lightning device selection, such as auto, 1, or a comma-separated device list."),
        integer("TrainingConfig.batch_size", 64, "Molecules per optimization batch.", 1, 1_000_000),
        integer("TrainingConfig.num_workers", 0, "Data-loading workers; zero is safest on macOS.", 0, 256),
        integer("TrainingConfig.num_epochs", 100, "Training epochs; when resuming, this is the number of additional epochs.", 1, 100_000),
        integer("TrainingConfig.early_stopping_patience", 20, "Epochs without improvement before stopping.", 0, 100_000),
        integer("TrainingConfig.warmup_epochs", 2, "Linear learning-rate warmup epochs.", 0, 100_000),
        number("TrainingConfig.init_lr", 1e-4, "Initial learning rate.", 0.0, 1.0),
        number("TrainingConfig.max_lr", 1e-3, "Peak learning rate.", 0.0, 1.0),
        number("TrainingConfig.final_lr", 1e-4, "Final learning rate.", 0.0, 1.0),
        integer("TrainingConfig.ensemble_size", 1, "Models trained and averaged per split.", 1, 1_000),
        integer("TrainingConfig.num_replicates", 1, "Repeated seeded runs; subsequent runs increment the data seed.", 1, 1_000),
        items("TrainingConfig.metrics", ["rmse", "mae", "r2"], "Official Chemprop regression metrics calculated during training."),
        choice("TrainingConfig.regression_loss", "mse", ("l2", "mse", "mae", "huber", "nll", "gaussian_nll"), "Regression optimization loss."),
        number("TrainingConfig.huber_delta", 1.0, "Huber quadratic-to-linear transition.", 1e-8, 1e6),
        number("TrainingConfig.nll_scale", 1.0, "Fixed Laplace scale for NLL loss.", 1e-8, 1e6),
        number("TrainingConfig.gaussian_nll_variance", 1.0, "Fixed variance for Gaussian NLL loss.", 1e-8, 1e6),
        text("TrainingConfig.tracking_metric", "val_loss", "Metric used for early stopping and checkpoint selection."),
        boolean("TrainingConfig.save_data_splits", True, "Save reproducible partition records (SMILES split files in Chemprop 2.2; full split files in 2.3+)."),
        text("TrainingConfig.executable", "chemprop", "Chemprop v2 executable in the active environment."),
        items("TrainingConfig.extra_args", [], "Additional official Chemprop CLI arguments."),
        boolean("TrainingConfig.dry_run", False, "Validate data and save the launch manifest without starting training."),
    ),
)
...
KERMT = ModelParameters(
    ...
    (
        ...
        choice("TrainingConfig.regression_loss", "l2", ("l2", "mse", "mae", "huber", "nll", "gaussian_nll"), "KERMT regression training loss."),
        number("TrainingConfig.huber_delta", 1.0, "Huber transition point.", 1e-8, 1e6),
        number("TrainingConfig.nll_scale", 1.0, "Fixed Laplace scale for NLL loss.", 1e-8, 1e6),
        number("TrainingConfig.gaussian_nll_variance", 1.0, "Fixed variance for Gaussian NLL loss.", 1e-8, 1e6),
